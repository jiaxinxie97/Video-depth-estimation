...
def compute_errors(gt, pred):
    ...
    log_sum=log_diff.sum()
    rmse = (gt - pred) ** 2
    rmse = np.sqrt(rmse.mean())

    rmse_log = (np.log(gt) - np.log(pred)) ** 2
    SILog=np.sqrt((rmse_log.mean())-((log_sum*log_sum)/(len(gt)*len(gt))))
    rmse_log = np.sqrt(rmse_log.mean())
    abs_rel = np.mean(np.abs(gt - pred)/gt)
    
    sq_rel = np.mean(((gt - pred)**2) /(gt))
    mean_error=np.mean(gt)
    irmse=(1.0/gt-1.0/pred)**2
    irmse=np.sqrt(irmse.mean())
#    mean_error=(np.abs(np.log(gt)-np.log(pred))).mean()
    return abs_rel, sq_rel, rmse, rmse_log, a1, a2, a3,SILog,irmse
...
#    print ('cam2cam:',cam2cam['P_rect_02'],'velo2cam',velo2cam['R'])
    ...
    velo = velo[velo[:, 0] >= 0, :]#remove points behind the image plane
    ...
    depth[depth<0] = 0
    #velo_disp= load_disparity_points(velo_file_name,color=False)
#    velo_disp=np.copy(velo_pts_im)
 #   velo_disp[:,2]=1.0/velo_disp[:,2]
   # velo_disp=load_disparity_points(velo_file_name)
    if interp:
...
'''
def load_disparity_points(velo_file_name, color=False, **kwargs):

            calib = Calib(color=color, **kwargs)
        
              # read velodyne points
            #points = load_velodyne_points(drive, frame, **kwargs)
            points=load_velodyne_points(velo_file_name)
